...
void  tsai3D_define_camera_parameters(int CameraModel, 
	...
	switch(CameraModel) {
	// - cameras
	// I calculated these based on the above and research on the iPhone 7 Camera
	// The iPhone 7 uses a IMX315 Sony Sensor and these values are based on that
	case IPHONE_7: 
		cp->Ncx = 4033;               /* [sel]        */
		cp->Nfx = 3024; 
		cp->dx = 0.00119;		/* [mm/sel]     */
		cp->dy = 0.00119;		/* [mm/sel]     */
